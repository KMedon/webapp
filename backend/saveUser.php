...
try {
    ...
    $stmt = $conn->prepare("INSERT INTO users (name, email, password, user_role, address, city, born_date) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if (!$stmt) {
        throw new Exception("Failed to prepare statement: " . $conn->error);
    }

    // Bind the parameters
    $stmt->bind_param("sssssss", $name, $email, $password, $role, $address, $city, $born_date);
